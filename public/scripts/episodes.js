...
        // Get a reference to the newly created h3 element
        const titleElement = div.querySelector('h3');
        // Call the new function to resize the font if needed
        resizeTitleFont(titleElement);
      });
...
// New function to resize font based on container width
function resizeTitleFont(titleElement) {
  // Get the parent container's width (the .episode div)
  const containerWidth = titleElement.parentElement.offsetWidth;
  
  // Use a while loop to check if the text overflows and adjust font size
  while (titleElement.scrollWidth > containerWidth) {
...
    // Add a safety break to prevent infinite loops for extremely long titles
